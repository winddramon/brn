...
class combat_dtsp extends combat_bra
{

	public function __construct(player_dtsp $att, player_dtsp $def)
	{
	...
	public function battle_start()
	{
		...
		do {
//			if (isset($defender->equipment['arb']['sk']['anti-' . strtoupper($this->weapon_kind($attacker))])) {
...
//		}
		...
		$damage *= $this->calc_damage_modulus($attacker, $defender);//把防各系武器的系数放到独立方法里去
		$modulus = 1;
	...
	protected function calc_damage_modulus(player $attacker, player $defender)//把伤害增减都堆到这里
	{
		global $g;
		//防具对伤害的影响，可以被攻击者的贯穿武器撤销
		$damage_modulus = 1;
		$atk_pierce_rate = 0;
		if(isset($attacker->equipment['wep']['sk']['atk-pierce'])){
			$atk_pierce_rate = $attacker->equipment['wep']['sk']['atk-pierce'];
		}
		if(!$g->determine($atk_pierce_rate)){//无贯穿或者贯穿失效才判断防具效果
			//盾牌效果
			if(isset($defender->equipment['ara']['sk']['shield'])){
				if($g->determine($defender->equipment['ara']['sk']['shield'])){
					$this->feedback($defender->name.' 的 '.$defender->equipment['ara']['n'].' 发动格挡，没有造成伤害');
					$damage_modulus = 0;
			...
			//防各系效果
			if(isset($defender->equipment['arb']['sk']['anti-'. strtoupper($this->weapon_kind($attacker)) ])){
				$anti_wep_rate = intval($defender->equipment['arb']['sk']['anti-'.strtoupper($this->weapon_kind($attacker))]);
				$this->feedback($defender->name.' 的 '.$defender->equipment['arb']['n'].' 抵消了 '.$anti_wep_rate.'% 的伤害！');
				$damage_modulus *= (100 - $anti_wep_rate) / 100;
			}

		}else{
			$this->feedback($attacker->name.' 的 '.$attacker->equipment['wep']['n'].' 贯穿了 '.$defender->name.' 的防具！');
		}
	...
	protected function modulus_defend(player $attacker, player $defender)
	{
		...
		if(isset($attacker->data['equipment']['wep']['sk']['atk-explode'])){//爆炸武器使防御力减少，参数为百分比
			$expl_modu = intval($attacker->data['equipment']['wep']['sk']['atk-explode']);
			if($expl_modu > 100){$expl_modu = 100;}
			elseif($expl_modu < 0){$expl_modu = 0;}
			$def *= (100 - $expl_modu) / 100;
			$this->feedback($attacker->data['equipment']['wep']['n'].'造成了爆炸伤害！'.$defender->name.'的防御力被削减了'.$expl_modu.'%！');
		}
