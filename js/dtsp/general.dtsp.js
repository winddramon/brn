...
$(document).ready(function(){
    ...
    $("#nav-cuser-logout").click(function(){
        $.post("ajax.dtsp.php", {action: "logout"}, function(data, status){
            if(!status){
    ...
    //Check if logged in
    $.post("ajax.dtsp.php", { action: "init" }, function(data, status){
        //data = eval('('+data+')');
        if(data['user'] != false){
    ...
    $("#nav-login #nav-login-form").submit(function(e){
        ...
        $.post("ajax.dtsp.php", { user: user, pass: pass, action: "login" }, function(data,status){
            if(!status){
                $("#nav-login div.output").html("网络错误");
                return;
